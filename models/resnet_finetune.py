...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        if self.channel == 3: #or self.channel == 15:
            x = self.dropout(x)
    ...
    def load_my_state_dict(self, path):
        ...
        for key, value in state_dict.items():
            ...
            if new_key != "fc.weight" and new_key != "fc.bias":
                pretrain_dict[new_key] = value
        
        # Replicate input channels of loaded state_dict for stack-of-differences
        model_dict = self.state_dict()
        model_dict = weight_transform(model_dict, pretrain_dict, self.channel)
...
def multiple_RGB_pretrain(conv1_weight, channel):
    # transform the original 3 channel weight to "channel" channel (must be multiple of 3)
    new_conv1_weight = torch.FloatTensor(64,channel,7,7)
    #print type(avg),type(new_conv1_weight)
    for i in range(channel):
        new_conv1_weight[:,i,:,:] = conv1_weight[:,i%3,:,:]
    return new_conv1_weight
...
def weight_transform(model_dict, pretrain_dict, channel):
    ...
    if channel == 3:
        ...
        wt = multiple_RGB_pretrain(w3,channel)
    else:
